...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    //MultiSampling Anti-Aliasing的英文缩写，指多重采样抗锯齿
    bool MSAA = true;      //是否开启MSAA，也就是是否用MultiSampling处理Anti-aliasing, 
    ...
    if(MSAA)
    {
        ...
        for(int x = min_x; x <= max_x; x++){
            for(int y = min_y; y <= max_y; y++){                
                int count = 0;  //记录一个像素（当前一个像素内超采样4个点）内部有多少个点在三角形内
                float minDepth =  FLT_MAX;  //并记录在三角形内的点里对应深度值的最小值
                for(int i = 0; i < 4; i++){
                ...
                if(count!=0){
                    if(depth_buf[get_index(x, y)] > minDepth){
                        ...
                        //Vector3f color = t.getColor() * count / 4.0f + (4 - count) * frame_buf[get_index(x,y)] / 4.0f;
                        Vector3f point(3);
